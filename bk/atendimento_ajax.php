...
try {
    ...
    function conectarBanco() {
        ...
        if ($conexao->connect_error) {
            ...
            $erro = sprintf(
                'Erro ao conectar ao banco: %s - Host: %s - Usuário: %s - Banco: %s',
            ...
            die(json_encode(['sucesso' => false, 'mensagem' => $erro]));
        }
...
switch ($acao) {
    ...
    case 'iniciar_atendimento':
        $id = intval($_POST['id_paciente'] ?? 0);
        if ($id > 0) {
            atualizarStatus($id, 'em_atendimento');

            // Atualiza também na sessão
            if (isset($_SESSION['fila_atendimento'])) {
                foreach ($_SESSION['fila_atendimento'] as $key => $paciente) {
                    if ($paciente['id'] == $id) {
                        $_SESSION['fila_atendimento'][$key]['status'] = 'em_atendimento';
...
function listarFila() {
    ...
    $sql = "
        SELECT 
            ...
            CASE 
                ...
                WHEN prioridade = 'Preferencial' THEN 2
                ELSE 3 
    ...
    if ($resultado === false) {
        respostaErro('Erro ao buscar fila: ' . $conexao->error, 500);
    }
...
function adicionarFila() {
    ...
    $stmt = $conexao->prepare("
        ...
        VALUES (?, ?, ?, ?, CURDATE(), CURTIME(), 'aguardando')
    ");
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode([
            ...
            'paciente' => [
                ...
                'timestamp' => strtotime($paciente['timestamp'])
            ]
        ]);
    } else {
