...
-- Compatível com MySQL 5.1+ (PHP 5.4)
-- ============================================

-- Criar banco de dados
CREATE DATABASE IF NOT EXISTS `totem_saude` 
...
INSERT INTO `tipos_atendimento` (`nome`, `descricao`, `prioridade_padrao`, `tempo_medio_atendimento`) VALUES
...
INSERT INTO `salas` (`numero`, `tipo`, `descricao`) VALUES
...
INSERT INTO `profissionais` (`nome`, `especialidade`, `crm`) VALUES
...
INSERT INTO `atendentes` (`nome`, `usuario`, `senha`, `email`, `nivel_acesso`) VALUES
...
INSERT INTO `pacientes` (`nome`, `cpf`, `data_nascimento`, `matricula`) VALUES
...
CREATE VIEW `v_fila_atendimento` AS
SELECT 
    sa.id,
    sa.numero_senha,
    sa.prioridade,
...
CREATE FUNCTION gerar_proxima_senha(p_tipo CHAR(1))
RETURNS VARCHAR(10)
READS SQL DATA
DETERMINISTIC
BEGIN
...
CREATE INDEX idx_senhas_data_status_prioridade ON senhas_atendimento (data_atendimento, status, prioridade);
CREATE INDEX idx_agendamentos_data_status ON agendamentos (data_agendamento, status);
CREATE INDEX idx_logs_data_acao ON logs_sistema (data_acao);

-- ============================================
-- GRANTS E PERMISSÕES (AJUSTE CONFORME NECESSÁRIO)
-- ============================================

-- GRANT SELECT, INSERT, UPDATE ON totem_saude.* TO 'totem_user'@'localhost' IDENTIFIED BY 'senha_segura';
